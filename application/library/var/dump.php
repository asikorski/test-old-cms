...
class var_dump {
	...
	//constructor
	function var_dump($var,$forceType="",$bCollapsed=false) {
	...
	function makeTableHeader($type,$header,$colspan=2) {
		...
		$this->html= $this->html."<table cellspacing=2 cellpadding=3 class=\"dBug_".$type."\">
				<tr>
	...
	function checkType($var) {
		switch(gettype($var)) {
			...
			default:
				...
				$this->html= $this->html. "<table cellspacing=0><tr>\n<td>".$var."</td>\n</tr>\n</table>\n";
				break;
		}
	}
	...
	function varIsArray($var) {
		...
		if(is_array($var)) {
			foreach($var as $key=>$value) {
				...
				$this->html= $this->html. $this->closeTDRow();
			}
		}
		else $this->html= $this->html. "<tr><td>".$this->error("array").$this->closeTDRow();
	...
	function varIsObject($var) {
		...
		if(is_object($var)) {
			...
			foreach($arrObjVars as $key=>$value) {
				...
				if(in_array(gettype($value),$this->arrType))
					$this->checkType($value);
				else $this->html= $this->html. $value;
				$this->html= $this->html. $this->closeTDRow();
			}
			$arrObjMethods=get_class_methods(get_class($var));
			foreach($arrObjMethods as $key=>$value) {
				$this->makeTDHeader("object",$value);
				$this->html= $this->html. "[function]".$this->closeTDRow();
			}
		}
		else $this->html= $this->html. "<tr><td>".$this->error("object").$this->closeTDRow();
		array_pop($this->arrHistory);
		$this->html= $this->html. "</table>";
	}
	...
	function varIsResource($var) {
		...
		switch(get_resource_type($var)) {
			...
			case "xml":
				...
				break;
			default:
				$this->html= $this->html. get_resource_type($var).$this->closeTDRow();
				break;
		}
		$this->html= $this->html. $this->closeTDRow()."</table>\n";
	}
	...
	function varIsDBResource($var,$db="mysql") {
		...
		$this->html= $this->html. "<tr><td class=\"dBug_resourceKey\">&nbsp;</td>";
		for($i=0;$i<$numfields;$i++) {
			...
			$this->html= $this->html. "<td class=\"dBug_resourceKey\" title=\"".$field_header."\">".$field_name."</td>";
		}
		$this->html= $this->html. "</tr>";
		for($i=0;$i<$numrows;$i++) {
			$row=call_user_func($db."_fetch_array",$var,constant(strtoupper($db)."_ASSOC"));
			$this->html= $this->html. "<tr>\n";
			$this->html= $this->html. "<td class=\"dBug_resourceKey\">".($i+1)."</td>";
			for($k=0;$k<$numfields;$k++) {
				$tempField=$field[$k]->name;
				$fieldrow=$row[($field[$k]->name)];
				$fieldrow=($fieldrow=="") ? "[empty string]" : $fieldrow;
				$this->html= $this->html. "<td>".$fieldrow."</td>\n";
			}
			$this->html= $this->html. "</tr>\n";
		}
		$this->html= $this->html. "</table>";
	...
	function varIsGDResource($var) {
		...
		$this->html= $this->html. imagesx($var).$this->closeTDRow();
		$this->makeTDHeader("resource","Height");
		$this->html= $this->html. imagesy($var).$this->closeTDRow();
		$this->makeTDHeader("resource","Colors");
		$this->html= $this->html. imagecolorstotal($var).$this->closeTDRow();
		$this->html= $this->html. "</table>";
	}
	...
	function varIsXmlResource($var) {
		...
		//if xml is not a file, attempt to read it as a string
			...
			if(!is_string($var)) {
				$this->html= $this->html. $this->error("xml").$this->closeTDRow()."</table>\n";
				return;
			}
			$data=$var;
	...
	function xmlStartElement($parser,$name,$attribs) {
		...
		$this->xmlSData[$this->xmlCount].='$this->html= $this->html. "<strong>'.$this->xmlName[$this->xmlCount].'</strong>".$this->closeTDRow();';
		...
		if(count($attribs)>0)
			...
			$this->xmlSData[$this->xmlCount].='$this->html= $this->html. "&nbsp;";';
		$this->xmlSData[$this->xmlCount].='$this->html= $this->html. $this->closeTDRow();';
		$this->xmlCount++;
	}
	...
	function xmlEndElement($parser,$name) {
		for($i=0;$i<$this->xmlCount;$i++) {
			...
			$this->html= $this->html. (!empty($this->xmlCData[$i])) ? $this->xmlCData[$i] : "&nbsp;";
			$this->html= $this->html. $this->closeTDRow();
			$this->makeTDHeader("xml","xmlComment");
			$this->html= $this->html. (!empty($this->xmlDData[$i])) ? $this->xmlDData[$i] : "&nbsp;";
			$this->html= $this->html. $this->closeTDRow();
